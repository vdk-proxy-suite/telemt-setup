...
if [[ "$legacy" != true ]]; then
  ...
  exec python3 "${ROOT_DIR}/tools/instance.py" cleanup "${forward[@]}"
fi
set -- "${forward[@]}"
...
if [[ -f "$MANIFEST" ]]; then
  # Never evaluate or adopt paths from an old shell manifest.
  UFW_RULE_ADDED=0
  grep -qx "UFW_RULE_ADDED=1" "$MANIFEST" && UFW_RULE_ADDED=1 || true
fi
